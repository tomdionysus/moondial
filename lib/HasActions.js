	...
	do(actor, cmd, param) {
		...
		if(this.before[cmd] && this.before[cmd].length>0) {
			for(i in this.before[cmd]) {
				if(this.before[cmd][i].call(this,actor,param)) return true
			}
		}
		if(this.actions[cmd].call(this,actor,param)) return true
		if(this.after[cmd] && this.after[cmd].length>0) { 
			for(i in this.after[cmd]) {
				if(this.after[cmd][i].call(this,actor,param)) return true
			}
		}
	},
